...
exports.graphVaccinationCountries = (req, res) => {
    let country = req.params.country;
    graphService.giveVaccinationCountries(country, (error, results)=>{
        if(error){
            console.log(error);
            return res.status(400).send({success: 0, data: error});
        }
...
exports.graphWorldMapCases = (req, res) => {
    ...
    graphService.getWorldMapCases(date,(error, results)=>{
        if(error){
            console.log(error);
            return res.status(400).send({success: 0, data: error});
        }
...
exports.graphHeatMap = (req, res) => {
    let vaccine = req.params.vaccine;
    graphService.getHeatmapData(vaccine, 10,(error, results)=>{
        if(error){
            console.log(error);
            return res.status(400).send({success: 0, data: error});
        }
...
exports.graphHeatMapDetailed = (req, res) => {
    let vaccine = req.params.vaccine;
    graphService.getHeatmapData(vaccine, 1, (error, results)=>{
        if(error){
            console.log(error);
            return res.status(400).send({success: 0, data: error});
        }
